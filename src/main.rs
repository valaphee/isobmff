...
use windows::{
    ...
    Win32::{
        ...
        Graphics::{
            Direct3D11::{
                D3D11CreateDevice, ID3D11Texture2D, D3D11_CPU_ACCESS_READ, D3D11_SDK_VERSION,
                D3D11_USAGE_STAGING,
            },
            Dxgi::{CreateDXGIFactory1, IDXGIFactory1, IDXGIOutput1, IDXGISurface1, DXGI_MAP_READ},
        },
    },
};

use mp4::marshall::{
    ChunkOffsetBox, Encode, FileTypeBox, HandlerBox, MediaBox, MediaDataBox, MediaHeaderBox,
    MediaInformationBox, MediaInformationHeader, MovieBox, MovieHeaderBox, SampleDescriptionBox,
    SampleSizeBox, SampleTableBox, SampleToChunkBox, SampleToChunkEntry, SyncSampleBox,
    TimeToSampleBox, TimeToSampleEntry, TrackBox, TrackHeaderBox, VisualSampleEntry,
};
...
impl<W: Write + Seek> Mp4Writer<W> {
    ...
    fn write_footer(mut self) -> W {
        ...
        self.writer
            .seek(SeekFrom::Start(self.media_data_start))
            .unwrap();
        self.writer
            .write_u32::<BigEndian>(media_data_size as u32)
            .unwrap();
                        ...
                        sample_table: SampleTableBox {
                            description: SampleDescriptionBox {
                                visual: Some(VisualSampleEntry {
                                    ...
                                    depth: 24,
                                    config: vec![
                                        0, 0, 0, 29, 97, 118, 49, 67, 129, 12, 12, 0, // av1C
                                        10, 15, 0, 0, 0, 98, 234, 127, 236, 251, 181, 242, 32, 33,
                                        160, 48, 128, // OBU_SEQUENCE_HEADER
                                    ],
                                }),
                                audio: None,
                            },
...
fn main() {
    ...
    std::thread::spawn(move || unsafe {
        ...
        device
            .CreateTexture2D(&copy_texture_desc, None, Some(&mut copy_texture))
            .unwrap();
        ...
        for _ in 0..60 * 8 {
            ...
            let bgra = std::slice::from_raw_parts_mut(
                mapped_rect.pBits,
                (copy_texture_desc.Width * copy_texture_desc.Height * 4) as usize,
            );
    ...
    loop {
        match packet_receiver.recv() {
            Ok(packet) => {
                writer.write_sample(&packet.data, 32, packet.frame_type == FrameType::KEY)
            }
            Err(_) => break,
        }
    }
